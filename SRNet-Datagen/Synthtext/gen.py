...
class datagen():
    ...
    def __init__(self):
        ...
        temp_bg_filepath = os.path.join(cur_file_path, data_cfg.temp_bg_path)


        self.bg_list = [temp_bg_filepath+img_path.strip() for img_path in os.listdir(temp_bg_filepath) if not img_path.startswith('.')]
    ...
    def gen_srnet_data_with_background(self):
        ...
        while True:
            ...
            name_list, title_list = build_vocab.build_corpus()

            name1 = build_vocab.generate_name(title_list, name_list)
            name2 = build_vocab.generate_name(title_list, name_list)

            # global cnt
            # if cnt < 48550:
            #     with open('/Users/hongzhi/Desktop/SRNet/SRNet-Datagen/Synthtext/thai_5w_thrid_parity_predict_names.csv') as f:
            #         name_list = f.readlines()[1:]
            #         name1 = name_list[cnt].split(',')[-1]
            #         name2 = name_list[cnt].split(',')[-1]
            # else:
            #     name_list, title_list = build_vocab.build_corpus()
            #
            #     name1 = build_vocab.generate_name(title_list, name_list)
            #     name2 = build_vocab.generate_name(title_list, name_list)
            #
            # with open("/Users/hongzhi/Desktop/SRNet/label.txt", 'a') as f:
            #     f.writelines("%d.jpg\t%s\n" % (cnt, name2))
            #
            # cnt += 1


            # bg = cv2.imread(random.choice(self.bg_list))
            bg = cv2.imread("/Users/hongzhi/Desktop/others/work/SRNet/SRNet-Datagen/Synthtext/datasets/bg_data/bg_img/0.jpg")
            # b, g, r = cv2.split(bg)
            # bg = cv2.merge([r, g, b])
            ...
            param = {
                        ...
                        'curve_center': np.random.randint(0, len(name1))
                    }
            surf1, bbs1 = render_text_mask.render_text(font, name1, param)
            param['curve_center'] = int(param['curve_center'] / len(name1) * len(name2))
            surf2, bbs2 = render_text_mask.render_text(font, name2, param)
            ...
            i_t = render_standard_text.make_standard_text(self.standard_font_path, name2, (surf_h, surf_w))
            ...
            # get font color
            fg_col, bg_col = (np.array([26, 25, 25])).astype(np.uint8), (np.array([255, 255, 255])).astype(np.uint8)
...
class multiprocess_datagen():
    ...
    def multiprocess_runningqueue(self):
        ...
        for _ in range(self.process_num):
            p = self.pool.apply_async(self.enqueue_data, args = (self.queue, self.data_capacity))
            self.processes.append(p)
        self.pool.close()

    def enqueue_data(self, queue, capacity):
